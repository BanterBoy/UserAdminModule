class Greetings {
    ...
    Greetings() {
        ...
        $jsonFilePath = "$PSScriptRoot\..\Resources\greetings.json"
    ...
    [void] Process([string] $name) {
        ...
        if ($this.greetings.ContainsKey($name)) {
            ...
            if ($this.asciiMappings.ContainsKey($name)) {
                ...
                Write-Output ($this.asciiMappings[$name] | Set-Clipboard -PassThru)
            }
            else {
...
function New-Greeting {
    ...
    .PARAMETER Day
    The day of the week for which the greeting message should be created. If not specified, the current day is used.
