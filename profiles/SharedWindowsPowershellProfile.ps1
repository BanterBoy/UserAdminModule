...
<#
.SYNOPSIS
    Shared Windows PowerShell 5.1 profile for UserAdminModule users.
.DESCRIPTION
    Dot-source this file from your personal Windows PowerShell profile ($PROFILE) to get:
    ...
    Repo clone — add to $PROFILE:
        . "C:\Path\To\UserAdminModule\profiles\SharedWindowsPowershellProfile.ps1"
...
# ── PS5 Desktop polyfill for $isWindows / $isLinux / $isMacOS ────────────────
...
if (Get-Module -ListAvailable -Name UserAdminModule -ErrorAction SilentlyContinue) {
    ...
    # Repo clone: this file lives at <ModuleRoot>\profiles\SharedWindowsPowershellProfile.ps1
